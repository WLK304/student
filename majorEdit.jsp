...
<!DOCTYPE html>
<html><head><meta charset="UTF-8"><title>修改专业</title>
<style>
...
<body>
<div class="header"><h1>📚 修改专业</h1></div>
<div class="container">
    <form action="${pageContext.request.contextPath}/MajorServlet?action=update" method="post">
        <input type="hidden" name="id" value="<%= major.getId() %>">
        <div class="form-group"><label>专业编号：</label><input type="text" name="majorNo" value="<%= major.getMajorNo() %>" required></div>
        <div class="form-group"><label>专业名称：</label><input type="text" name="majorName" value="<%= major.getMajorName() %>" required></div>
        <div class="form-group"><label>专业描述：</label><textarea name="description"><%= major.getDescription() != null ? major.getDescription() : "" %></textarea></div>
